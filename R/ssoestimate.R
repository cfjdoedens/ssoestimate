...
#'
#' @returns A list of named elements containing:
#'
#'     - the probability graph of the error fraction in SSO, P_SSO
#'     - the probability graph of the error fraction in Steek_client, P_client
#'     - k_SSO
#'     - n_SSO
#'     - n_client
#'     - S_p_SSO
#'     - S_k_client (== NA, is for compatibility with SSO_estimate_continuous())
#' @export
#'
#' @examples
#' SSO_estimate(k_SSO = 1, n_SSO = 3000, n_client = 500, S_p_SSO = 1000)
#'
SSO_estimate <- function(k_SSO = 0,
  ...
  for (p_i in rooster) {
    ...
    #
    # Gegeven p_i
    # berekenen we voor alle mogelijke gehele waarden van k_client de waarschijnlijkheid van dit
    ...
    P_client_i <-
      dbinom(k_SSO, n_SSO, p_i) * stats::dbinom(K_client, n_client, p_i)
  ...
  list(
    ...
    "n_client" = n_client,
    "S_p_SSO" = S_p_SSO,
    "S_k_client" = NA # For compatibility with SSO_estimate_continous().
  )
}

#' Fine grained estimation of error fraction in SSO and client.
#'
#' @param k_SSO The number of errors found in Steek_SSO. k_SSO >= 0.
#' @param n_SSO The number of posten in Steek_SSO. n_SSO > 0.
#' @param n_client The number of posten in Steek_client. n_client > 0.
...
#'
#' @examples
#' y <- SSO_estimate_continous(k_SSO = 1, n_SSO = 3000, n_client = 500,
#'                             S_p_SSO = 1000, S_k_client = 501)
#'
SSO_estimate_continous <- function(k_SSO,
                                   n_SSO,
                                   n_client,
                                   S_p_SSO = 1000,
                                   S_k_client = 1000) {
  # Versie waar we met een fijner rooster werken:
  # we werken met nietgehele waarden voor k_client.
...
SSO_graph_plot <-
  function(SSO_out,
    ...
    if (visual) {
      ...
      # Construct subtitle.
      line1 <- sprintf("SSO    most probable %1.5f min %1.5f (at cert %1.2f) max %1.5f (at cert %1.2f)",
                       SSO_most_prob,
                       SSO_min,
                       cert,
                       SSO_max,
                       cert)
      line2 <- sprintf("client  most probable %1.5f min %1.5f (at cert %1.2f) max %1.5f (at cert %1.2f)",
                       client_most_prob,
                       client_min,
                       cert,
                       client_max,
                       cert)
      if (is.na(S_k_client)) { # S_k_client is missing.
        line3 <- sprintf(
      ...
      ggplot(data = t) +
        ...
        # The same for ‘who’.
        geom_point(mapping = aes(x = p, y = .data$prob, color = .data$who)) +
        scale_color_manual(
          labels = c("client", "SSO"),
